...
<html lang="cs">
  ...
  <body>
	...
	<script type="text/javascript">
 	...
 	<?php
		if(isset($_POST['submitBtn']))
		{
			...
			$sql="INSERT INTO clanky (titulek,kratkyPopis,clanek,datum) VALUES (?,?,?,?)";
			...
			if(!mysqli_stmt_prepare($stmt,$sql)){
				...
				mysqli_stmt_bind_param($stmt, "ssss", $titulekText,$popisText,$clanekText,$datum);
	...
	<?php
		...
		if(strpos($celaURL,"zmenaClanku=uspesna") == true){
			echo '<div class = "successHlaska">';
			echo "<p class='success'>Článek byl úspěšně upraven!</p>";
			echo '</div>';
		}
		else if(strpos($celaURL,"error=nezadanytitulek") == true){
			echo '<div class = "errorHlaska">';
			echo "<p class='error'>Titulek nebyl zadán!</p>";
			echo '</div>';
		}
		else if(strpos($celaURL,"error=nezadanypopis") == true){
			echo '<div class = "errorHlaska">';
			echo "<p class='error'>Krátký popis nebyl zadán!</p>";
			echo '</div>';
		}
		else if(strpos($celaURL,"error=nezadanyclanek") == true){
			echo '<div class = "errorHlaska">';
			echo "<p class='error'>Nebyl zadán text článku!</p>";
			echo '</div>';
		}
    ?>
	<form method="post" action="../php_soubory/upravitClanek.php" id="textEditor">
	  <input type="text" class="form-control" style="margin-bottom:10px; width:300px;" placeholder="Titulek" name="textTitulek" id="titulek" value="<?php echo $_SESSION['titulekClanku'];?>">
	  <input type="text" class="form-control" style="margin-bottom:10px; width:500px;" placeholder="Krátký popis" name="textPopis" id="popis" value="<?php echo $_SESSION['kratkyPopisClanku'];?>">
      <textarea name="content" style="width:100%">
	  <?php
		echo $_SESSION['clanekSamotny'];	  
	  ?>
      </textarea>
	  <button name="submitUpravu" style="margin-top:10px;margin-bottom:20px;width:150px;">Upravit</button>
	</form>
  </body>
</html>
